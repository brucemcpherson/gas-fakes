# Version 2.3.15 Release Notes

## Summary
Version 2.3.15 brings critical parity enhancements to both the `GmailApp` and `SpreadsheetApp` (Charts) services, ensuring scripts run locally on Node.js behave identically to Live Apps Script. This release also introduces significant improvements to the `gf_agent` knowledge base and its autonomous orchestration capabilities.

## Features and Enhancements

### 1. Gmail Service Completions
The `GmailApp`, `GmailMessage`, `GmailThread`, `GmailDraft`, and `GmailLabel` classes have been fully implemented to achieve 100% method coverage based on the official Apps Script documentation.
- **Bulk Operations**: Implemented `GmailApp.markMessagesRead()`, `moveThreadsToTrash()`, `starMessages()`, and other batch modifiers.
- **Drafts and Attachments**: Fixed draft creation via `createDraftReply()` and `createDraftReplyAll()`. Added full support for `getAttachments()` and `getAs()` on `GmailAttachment`.
- **Label Management**: Added full support for label assignment and querying (`addToThread`, `getThreads`, `getUnreadCount`).
- **Worker Thread Dot-Notation Support**: Upgraded the `sxGmail` background worker bridge to correctly resolve deeply nested Google API endpoints (e.g., `messages.attachments.get`), bypassing previous limitations.

### 2. SpreadsheetApp Additions & Chart Visual Methods
- **SpreadsheetApp.open(file)**: Fully implemented the ability to open a spreadsheet by passing a Drive `File` object.
- **Implemented Chart Methods**: `setColors()`, `setXAxisTitle()`, `setYAxisTitle()`, `setRange()`, `setXAxisRange()`, `setYAxisRange()`, `setStacked()`, `setBackgroundColor()`, `setLegendPosition()`, and `set3D()`.
- **API Parity Alignment**: Stub methods that do not have a 1:1 mapping in the modern v4 API (e.g., `useLogScale()`, `*TextStyle()`) have been explicitly flagged internally as `notYetImplemented` to warn developers when their charts will not receive that specific formatting.

### 3. Agent Orchestration & Knowledge (`gf_agent`)
The `gf_agent` has received significant knowledge upgrades regarding the "Oddities" of the Live Apps Script environment:
- **Sandbox Security Awareness**: The agent is now fully trained on how to configure and execute tasks securely within the `gas-fakes` sandbox mode, utilizing file whitelists, email whitelists, and granular usage limits.
- **Eventual Consistency Pattern**: The agent now understands that Live Apps Script Gmail modifiers (like `markRead()`) are eventually consistent despite the official documentation. It has been instructed to use the `Utilities.sleep()` and `.refresh()` pattern when writing cross-platform tests.
- **Chart Parity Traps**: The agent has been trained to avoid "Method Fragmentation" traps, such as attempting to use `setXAxisRange()` on a `ColumnChart` (which works in `gas-fakes` but crashes in Live GAS because it is technically exclusive to `ScatterCharts`).

## Bug Fixes

- **Live Apps Script Crash (`getSnippet`)**: Removed the `getSnippet()` method from `FakeGmailMessage`. This method existed in the REST API but not in the Apps Script V8 runtime, causing parity tests to falsely assert its existence and crash on live Google servers.
- **Draft Counting Limit**: Fixed a test suite bug where draft creation assertions would fail if the user's Gmail account had hit the pagination limit of 500 drafts.
- **Chart Range Coercion**: Fixed a bug where `setRange(min, max)` incorrectly forced the `LEFT_AXIS` range on `BAR` charts, instead of adapting to the `BOTTOM_AXIS`.
