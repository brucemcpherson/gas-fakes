# Version 2.3.16 Release Notes

## Summary
Version 2.3.16 focuses on completing coverage for DocumentApp container traversal methods.

## Features and Enhancements

### 1. DocumentApp Container Methods
Implemented the core retrieval methods on `FakeContainerElement`, allowing scripts to traverse and extract specific element types from any container (Body, Header, Footer, TableCell, etc.).
- **`getImages()`**: Retrieves all `InlineImage` objects within a section, intelligently parsing through paragraphs to locate inline image blocks.
- **`getListItems()`**: Recursively retrieves all `ListItem` objects.
- **`getParagraphs()`**: Recursively retrieves all `Paragraph` objects (which also includes `ListItem` objects, mirroring Live GAS behavior).
- **`getTables()`**: Recursively retrieves all `Table` objects.
