...
- added support for `insertTextBox()` and `insertTable()` to `Class Slide`.
- added classes `FakeTable`, `FakeTableCell`, and `FakeTableRow` to support table operations in slides.
- updated `FakePageElement`, `FakeElementFactory`, and `FakeTextRange` to integrate new slide elements.
