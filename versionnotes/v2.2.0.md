# v2.2.0 Release Notes - Breaking Change: Removal of gasfakes.json

This version introduces a significant breaking change in how `gas-fakes` is configured. The legacy `gasfakes.json` configuration file has been removed in favor of standard environment variables, typically managed within a `.env` file.

## What's Changed?

### 1. Removal of `gasfakes.json`
`gas-fakes` no longer reads from or writes to `gasfakes.json`. All settings previously held in this file are now provided via environment variables.

### 2. New Environment Variables
The following variables now control the behavior of `gas-fakes`:

| Legacy `gasfakes.json` Property | New Environment Variable | Default |
|---|---|---|
| `manifest` | `GF_MANIFEST_PATH` | `./appsscript.json` |
| `clasp` | `GF_CLASP_PATH` | `./.clasp.json` |
| `scriptId` | `GF_SCRIPT_ID` | from clasp, or random |
| `documentId` | `GF_DOCUMENT_ID` | `null` |
| `cache` | `GF_CACHE_PATH` | `/tmp/gas-fakes/cache` |
| `properties` | `GF_PROPERTIES_PATH` | `/tmp/gas-fakes/properties` |

### 3. Streamlined `init` Process
The `gas-fakes init` command has been updated to prompt for these paths and behavior settings during initialization. These are automatically saved to your `.env` file.

### 4. CLI Changes
The `-g, --gfsettings` option has been removed from the `gas-fakes` CLI as it is no longer relevant.

## Why this change?
Centralizing configuration in the `.env` file follows standard Node.js practices and simplifies environment management, especially when running in containers or CI/CD pipelines. It also removes the confusion of having multiple configuration files (`.env` and `gasfakes.json`).

## Action Required
If you have an existing project using `gasfakes.json`:
1.  Run `gas-fakes init` to migrate your settings to your `.env` file.
2.  You can safely delete your `gasfakes.json` file.
3.  Ensure your application is loading the `.env` file (e.g., using `dotenv` or the `node --env-file` flag).
