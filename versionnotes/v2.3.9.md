# v2.3.9 Version Notes

## Highlights
- **Introduction of the `gf_agent` Skill for Gemini CLI**: A specialized agent that understands Google Apps Script services and can automate tasks using natural language.
- **Built-in MCP Server**: Added a Model Context Protocol (MCP) server that exposes Google Workspace services (Sheets, Docs, Drive, Gmail, etc.) as tools for LLMs.
- **Automated Gemini CLI Integration**: Updated `gas-fakes init` to automatically link the `gf_agent` skill and configure the MCP server for Gemini CLI users.
- **Expanded Gmail Support**: Added several missing methods to `GmailMessage` and `GmailThread`, including `getSubject()`, `getDate()`, `getSnippet()`, `getFrom()`, `getTo()`, and `getMessages()`.

## Details

### Gemini CLI & MCP
- Added `src/cli/mcp.js` implementing a skills-based MCP server.
- Registered granular tools for each major Apps Script service.
- Implemented a general `workspace_agent` for complex, multi-service tasks.

### Core Library Updates
- **GmailMessage**: Implemented `getSubject()`, `getDate()`, `getSnippet()`, `getFrom()`, and `getTo()` by extracting header data from the Gmail API response.
- **GmailThread**: Implemented `getMessages()` to allow direct access to messages within a thread.

### CLI Enhancements
- Enhanced `gas-fakes init` to check for Gemini CLI and offer automatic installation of the `@gf_agent` skill and MCP configuration.

### Documentation
- Created `gf_agent/README.md` and `gf_agent/documentation.md`.
- Added a new descriptive article: `notes/gemini-skills-mcp.md`.
- Updated main `README.md` and `GETTING_STARTED.md` with information on the new agent features.
