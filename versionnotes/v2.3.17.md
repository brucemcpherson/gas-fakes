...
- **Utilities Service Parity**: Fully implemented all remaining missing methods in the `Utilities` service.
  - Added cryptographic signing methods: `computeHmacSignature`, `computeRsaSignature`, `computeRsaSha1Signature`, and `computeRsaSha256Signature`.
  - Added formatting and parsing utilities: `formatString`, `formatDate`, `parseCsv`, and `parseDate`.
  - Successfully mapped Enum properties (`Charset`, `DigestAlgorithm`, `MacAlgorithm`, `RsaAlgorithm`) to match Google Apps Script behavior.
  - Strict adherence to Java-based parity behavior (e.g., removing Node's `%j` string formatting from `formatString`).
- **EmbeddedChartBuilder Parity**: Enhanced `addRange()` to automatically split multi-column ranges into domain and series components, matching live Google Apps Script behavior and reducing script generation retries.

## Agent Knowledge Update
- **gf_agent Knowledge Base**: 
  - Added `14-utilities-parity.md` to capture specific API parity constraints around Apps Script's `Utilities` service.
  - Added `15-logging-efficiency.md` to provide instructions on reducing redundant documentation lookup tool calls and minimizing terminal logging.
  - Updated `05-sheets-forms.md` with new chart range guidance.
