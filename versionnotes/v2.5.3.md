# gas-fakes v2.5.3 Release Notes

## Overview
Version 2.5.3 focuses on achieving 100% parity for the Google Slides service, addressing critical text indexing bugs, and enhancing CLI automation for deployment workflows.

## Key Features & Additions

### 1. Slides Service Completion
- Achieved full documented parity for the Slides service, including Media, Fills, and Range classes.
- Implemented native `insertVideo()` methods in `Slide`, `Layout`, and `Master` classes.
- Standardized `batchUpdate` signatures across all 19+ Slides classes to ensure compatibility with both local emulation and live Google Apps Script.

### 2. Robust Text Indexing & Reconstruction
- Refactored `TextRange.asString()` to use a robust character-map reconstruction. This fix correctly handles `paragraphMarker` elements (implicit newlines) at their specific API indices, eliminating "leading newline" and "duplicate newline" bugs.
- Implemented safe index clamping in `insertText()` and `appendText()` to satisfy Slides API constraints regarding the final paragraph marker.
- Updated `setText()` and `clear()` to use the `ALL` range type for full-shape deletions, avoiding index-out-of-bounds errors on shapes with implicit trailing newlines.

### 3. CLI Deployment Automation (`--quiet` mode)
- Enhanced the `togas` deployment command with a new `-q, --quiet` flag.
- This flag bypasses interactive confirmation prompts by automatically accepting default actions (e.g., overwriting files or creating new projects), making it ideal for CI/CD pipelines and automated environments.

### 4. Documentation & Discovery Improvements
- Performed a global audit of the `progress/` and `notes/` directories, standardizing all filenames to lowercase `.md` and forcing Git to track these casing changes on macOS/Windows.
- Updated the documentation analyzer to correctly track implemented enums like `MimeType`.
- Documented "Slides Service Revision Isolation" in `oddities.md`, providing guidance on the "split-brain" behavior observed when mixing standard and advanced services in live GAS.

## Bug Fixes
- **Slides Advanced Service**: Fixed a critical caching regression in the `get` method by correctly passing sync method overrides during internal calls.
- **Advanced Service Payloads**: Updated `batchUpdate` and `create` to use `requestBody` (matching the Node.js `googleapis` client) while robustly handling both raw arrays and resource objects for GAS compatibility.
- **PageBackground**: Fixed property nesting and `setTransparent` logic to correctly map to `pageProperties.pageBackgroundFill`.
