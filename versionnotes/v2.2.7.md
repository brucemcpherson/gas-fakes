...
## Bug Fixes
- **ADC Scopes Preservation**: Fixed an issue where the `gcloud auth application-default login` command would omit the `--scopes` flag when only default scopes were requested. This previously caused essential scopes (like `drive.readonly`) to be omitted from the authentication request, resulting in `403 Insufficient authentication scopes` errors when attempting to access Google Drive APIs. The CLI now ensures the `--scopes` flag is always explicitly passed during ADC authentication to guarantee all requested scopes are correctly applied.
