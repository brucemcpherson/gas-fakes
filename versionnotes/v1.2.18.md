- Updated init and gas-fakes auth commands - see - [gas fakes cli](gas-fakes-cli.md) for details
- Added QUIET=true to .env to suppress logging from main thread and worker thread
- For collaborators, test should now have its own .env file in the test folder - so testing should now be self contained
