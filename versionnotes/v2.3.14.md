...
*   **`gf_agent` Toolkit Robustness**: Refactored `gf_agent/scripts/builder.js` to dynamically use script-relative paths (`import.meta.url`). This prevents the build script from incorrectly accessing or creating junk directories (like nested `gf_agent/gf_agent/`) in parent folders when executed within a sparse Git clone or a detached environment.
*   **Documentation Pipeline (Broken Links Fix)**: Updated `doccreation/include-docs.js` to intelligently calculate the directory depth of target markdown files. The injected `_includes/further.md` block now correctly prefixes relative paths (e.g. `../`) so that links to the newly reorganized `notes/` directory resolve properly from anywhere in the repository.
