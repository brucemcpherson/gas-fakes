...
KSUITE_TOKEN_VALUE=$(grep -E '^KSUITE_TOKEN=' "$ENV_FILE" | cut -d '=' -f2 | tr -d '"\r')
...
if [ -z "KSUITE_TOKEN_VALUE" ]; then
  echo "KSUITE_TOKEN not found or is empty in $ENV_FILE."
else
  echo "exported: KSUITE_TOKEN"
  export KSUITE_TOKEN="$KSUITE_TOKEN_VALUE"
fi  
