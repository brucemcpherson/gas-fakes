...
export const signatureArgs = (received, method, objectType = 'Object') => {
  ...
  try {
    ...
    cliLogger.warn ("...warning failed signature check- probably an unsupported probe. probe of an enum - ignoring", args)
    passedTypes=[]
  }
...
export const advClassMaker = (props) => {
  ...
  capped.map((f, i) => {
    ...
    if (done.has(f)) {
      cliLogger.warn('....WARNING duplicate property  in advClassMaker', f)
    }
    done.add(f)
    ob['get' + f] = () => ob[props[i]]
    ob['set' + f] = (arg) => {
