...
export const isFolderOnly = (params) =>
  isRoot(params) ||
  (typeof params?.q === "string" &&
    params.q.includes(`mimeType = '${folderType}'`));

export const isFileOnly = (params) =>
  !isRoot(params) &&
  typeof params?.q === "string" &&
  params.q.includes(`mimeType != '${folderType}'`);

export const filterOut = (params, files) => {
  return isFolderOnly(params)
...
export const minSheetFields =
  "sheets(properties,basicFilter,bandedRanges,merges,data.rowData.values.dataSourceTable),dataSources,spreadsheetId,properties.title,spreadsheetUrl";
/**
 ...
 * @returns {string}
 */
export const getFakeType = (item) =>
  getFakeService(item) && item.__getFakeType();
 ...
 * @returns {Boolean}
 */
export const isGood = (response) =>
  response && Math.floor(response.status / 100) === 2;
...
export const argsMatchThrow = (items, mess = "") => {
  ...
  throw new Error(
    `The parameters (${passedTypes}) don't match the method ${mess}`,
  );
};
...
export const ssError = (response, method, ss) => {
  if (!isGood(response)) {
    if (ss) {
      throw new Error(
        `Unexpected error while getting the method or property ${method} on object SpreadsheetApp. API call failed with error: ${response?.error?.message || response?.statusText}`,
      );
    } else {
      // adv drive throws this one
      throw new Error(
        `GoogleJsonResponseException: API call to sheets.spreadsheets.${method} failed with error: ${response?.error?.message}`,
      );
    }
  }
  return response;
...
export const signatureArgs = (received, method, objectType = "Object") => {
  ...
  try {
    passedTypes = args
      .map((f) => (is.null(f) ? "null" : is(f)))
      .map((f) => (f === "null" ? f : capital(f)))
      .map((f) => (f === "Object" ? objectType : f));
  } catch (err) {
    slogger.warn(
      "...warning failed signature check- probably an unsupported probe. probe of an enum - ignoring",
      args,
    );
    passedTypes = [];
  }
  const matchThrow = (mess = method) => {
    throw new Error(
      `The parameters (${passedTypes.join(",")}) don't match the method signature for ${mess}`,
    );
  };
  return {
    nargs,
 ...
 */
export const normalizeSerialization = (ob) =>
...
export const gError = (response, service, method, throw404 = false) => {
  if (!response) {
    throw new Error(
      `GoogleJsonResponseException: API call to ${service}.${method} failed with error: No response`,
    );
  }
  if (response.error) {
    if (throw404 || response.error.code !== 404) {
      throw new Error(
        `GoogleJsonResponseException: API call to ${service}.${method} failed with error: ${response?.error?.message}`,
      );
    }
  }
};
